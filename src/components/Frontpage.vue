...
@Component({
    ...
    sockets: {
        ...
        created: function(data: any): void{
            this.$store.commit('connection', {admin: true, roomId: data.roomId, songList: [], type: data.type})
...
export default class Frontpage extends Vue {
    ...
    mounted() {
        this.setContentSize();
        let indexedDB = window.indexedDB || (window as any).mozIndexedDB || (window as any).webkitIndexedDB || (window as any).msIndexedDB || (window as any).shimIndexedDB;
    ...
    connectionHandler(data: any) {
        ...
        if(data.status.code === 200) {
            this.connected({status: data.status.code, data: {roomId: data.room.roomId, type: data.room.type, songs: data.room.songs, nowPlaying: data.room.nowPlaying}})
        } else {
    ...
    connected(data: any) {
        if(data.status === 200) {
            ...
            this.$store.commit('connection', {admin: admin, roomId: data.data.roomId, songList: data.data.songs, type: data.data.type})
            this.$store.commit('nowPlaying', data.data.nowPlaying);
            saveRoomData({roomId: data.data.roomId, type: data.data.type, admin: admin});
            this.$router.replace({ name: "partybox" });
        }
    };
