...
@Component({
  ...
  computed: {
	sortedList() {
		...
		const pointsFirst = first.points;
		const pointsSecond = second.points;
	...
	sortByDate() {
		this.songlist.sort(function(first: Song, second: Song) {
		const idFirst = first.id;
		const ifSecond = second.id;
  ...
  sockets: {
	voted(data) {
		...
		for (const song of this.$props.songlist) {
		if (song.id === songId) {
...
export default class SongList extends Vue {
  ...
  public votingEvent(msg) {
	let song: Song | undefined;
	for (const songo of this.$props.songlist) {
		if (songo.id === msg.id) {
		song = songo;
		}
	}
